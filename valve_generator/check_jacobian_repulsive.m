...
[F_anterior F_chordae_left F_chordae_right] = difference_equations_repulsive(leaflet); 

F_linearized = linearize_internal_points(leaflet, F_anterior, F_chordae_left, F_chordae_right);

% jacobian does not change 
J = build_jacobian_replusive(leaflet); 
...
for i = 1:length(epsilon_vals)
    ...
    % eval the difference eqns on the perturbation 
    [F_perturbed F_chordae_left_perturbed F_chordae_right_perturbed] = difference_equations_repulsive(leaflet_perturbation); 
