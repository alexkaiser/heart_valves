...
def voxel_average_eulerian_to_mri_mesh(frame_number,
    ...
    print("Voxel avg, processing frame_number, ", frame_number)

    eulerian_name = eulerian_basename_orig + str(frame_number).zfill(digits_output_eulerian) + eulerian_extension
    print("Voxel avg, trying to read: ", eulerian_name)
    ...
    # all_scalars = True means all points must be within for a cell to stay 
    # all_scalars = False means any point must be within for a cell to stay 
    eulerian_mesh_centers_restricted = selected.threshold(0.5, scalars="SelectedPoints", all_scalars=True) 
    ...
    print("Voxel avg, writing file ", eulerian_name_out)
    eulerian_resampled.save(eulerian_name_out)
    ...
    if boundary_mesh_name is not None: 
        ...
        print("Voxel avg, writing restricted file ", eulerian_name_out_restricted)
...
def run_main():
    ...
    if run_voxel_avg:
        ...
        write_pvd(eulerian_basename_orig + suffix_eulerian + '_restricted', dt_mri_read, mri_read_times_per_cycle, output_extension, time_start_1=time_start_1)
