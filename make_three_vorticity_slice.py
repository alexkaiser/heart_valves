...
session_file = '/scratch/adk354/three_slice_' + str(n) + '_vorticity.session'
...
if not RestoreSessionWithDifferentSources(session_file, 0, data):
...
if (cwd_split[0]   == '') and (cwd_split[1]   == 'scratch') and (cwd_split[2]   == 'adk354') and (len(cwd_split) >= 4):
    base_name = cwd_split[3] + '_vorticity'
else:
    base_name = 'frames' + '_vorticity'
